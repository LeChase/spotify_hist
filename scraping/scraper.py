...
class Scraper:
    ...
    @staticmethod
    def get_from_json_genre(json_path):
        if os.path.isfile(json_path):
            df = pd.read_json(json_path, orient = 'table')
            set_index = set(df.index)
        else:
            df = pd.DataFrame(index = pd.Index([], name = ['artist']))
            set_index = set()
        return df, set_index
    ...
    def main_genre(self):
        ...
        unique_artists = Unique.get_unique_artists()
        ...
        # remove completed and failed artists from the set, and add those yet to be found
        unique_artists -= (completed_set | failed_set)
        unique_artists |= yet_set

        df = pd.DataFrame(index = pd.Index(unique_artists, name =  'artist'))
        df = df.sort_index()
        df[self.output_type] = np.nan
        ...
        time0 = time.time()
        n_artists = len(df_copy)

        # iterate over the original 'yet' dataframe
        # copied df is used for editing
        for i, artist in enumerate(df.index, 1):
            percent_done = round(100*i/n_artists, 3)
            dt = round(time.time() - time0)
            time_remaining = round(dt/i*(n_artists-i))
            print('[[{} / {} ({}%)]]'.format(i, n_artists, percent_done), \
                    '[[{}s elapsed ({}s remaining)]]'.format(datetime.timedelta(seconds = dt), 
                                                            datetime.timedelta(seconds = time_remaining)), \
                    artist)
            
            output = self.scrape_func(artist)
            if output == self.engine.failed_result:
                failed_df.at[artist, self.output_type] = output
                failed_df.to_json(failed_json, orient = 'table')
            else:
                completed_df.at[artist, self.output_type] = output
                completed_df.to_json(completed_json, orient = 'table')
            df_copy = df_copy.drop(song)
            df_copy.to_json(yet_json, orient = 'table')
